    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (    depth >= 3
          ...
          && (  !captureOrPromotion
              ...
              || thisThread->ttHitAverage < Tc[3] * TtHitAverageResolution * TtHitAverageWindow / 1024))
      {
          Depth r = reduction(improving, depth, moveCount);

          // Decrease reduction if the ttHit running average is large
          if (thisThread->ttHitAverage > Tc[4] * TtHitAverageResolution * TtHitAverageWindow / 1024)
              r--;
          ...
          if (captureOrPromotion)
          {
              ...
              if (   !givesCheck
                  && ss->staticEval + PieceValue[EG][pos.captured_piece()] + Tc[5] * depth <= alpha)
                  r++;
          }
          else
          {
